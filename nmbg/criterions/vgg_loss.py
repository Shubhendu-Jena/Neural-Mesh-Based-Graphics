...
class VGGLoss(nn.Module):
    def __init__(self, net='caffe', partialconv=False, optimized=True, save_dir='.cache/torch/models'):
        super().__init__()
        ...
        for i, layer in enumerate(self.vgg19):
            ...
            if i in self.layers:
                if i == 3:
                    ...
                    loss = loss + 0.75*F.l1_loss(features_input, features_target)
                elif i == 13:
                    loss = loss + 0.5*F.l1_loss(features_input, features_target)
                elif i == 22:
                    loss = loss + 0.5*F.l1_loss(features_input, features_target)
                elif i == 31:
                    loss = loss + F.l1_loss(features_input, features_target)
